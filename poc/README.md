...
2. **非獨立董監持股（PRD §8.10）50 檔中 47 檔 N/A。** FinMind 不提供，改由
   TWSE／TPEx 公開 open data（`t187ap11_L`／`mopsfin_t187ap11_O`，免金鑰）取得。
   本 PoC 環境的 IP 被證交所阻擋，只取得到上櫃資料，故僅信驊（5274）、
   環球晶（6488）、群聯（8299）三檔有分數，47 檔上市仍為 N/A；
   從一般網路環境執行即可補齊。未取得者一般產業可評滿分為 15，金融業為 8。
