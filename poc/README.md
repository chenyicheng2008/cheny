...
1. **「前 50」不是全市場前 50。** FinMind 免費／匿名層級不允許不帶 `data_id`
   的全市場查詢，程式無法對全市場排名。這 50 檔是在
   `config/universe_candidates.txt` 這個人工候選池（103 檔）內，
   由程式逐檔以 `收盤價 × 發行股數` 實測市值後取前 50。
   池內市值全是實測值，但池外是否有更大的公司無法驗證。
2. **非獨立董監持股（PRD §8.10）50 檔全部 N/A。** FinMind 不提供此資料。
   因此一般產業實際可評滿分為 15 而非 16，金融業為 8 而非 9。
...
資本支出僅含不動產廠房設備。詳見專案根目錄 README 的「PoC 實測結果」一節。
